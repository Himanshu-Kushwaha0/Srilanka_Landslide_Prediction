#!/usr/bin/env python
"""Test script for Landslide Prediction API."""
...
def test_health():
    ...
    print(f"Startup OK: {data.get('startup_ok')}")
...
def test_susceptibility():
    ...
    if r.status_code == 200:
        print(f"✓ Susceptibility map generated (size: {len(r.content)} bytes)")
    return r.status_code == 200
...
def test_model_run():
    ...
    if r.status_code == 200:
        ...
        print(f"✓ Model job submitted - Job ID: {job_id}")
        ...
        print("⚠ Timeout waiting for model to complete")
